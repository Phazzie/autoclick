...
class CredentialFilterAction(BaseAction):
    ...
    def _execute(self, context: Dict[str, Any]) -> ActionResult:
        ...
        message = self._get_variable(context, self.message_variable, "No message provided")

        # Check if the username is present
        if not username:
            return ActionResult.create_failure(f"Missing required variable: {self.username_variable}")
        ...
        updated_count = self.credential_manager.update_credentials_status(
            ...
            to_status=self.inactive_status
        )
    ...
    def _get_variable(self, context: Dict[str, Any], variable_name: str, default: Any = None) -> Any:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> 'CredentialFilterAction':
        ...
        inactive_status = CredentialStatus[inactive_status_name] if inactive_status_name in CredentialStatus.__members__ else CredentialStatus.BLACKLISTED
