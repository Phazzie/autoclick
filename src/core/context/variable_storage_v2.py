...
import re
import logging
from typing import Dict, Any, Optional, List, Callable, Union, Set, TypeVar, cast

from src.core.context.variable_storage import VariableScope
from src.core.variables.variable_interface import VariableType, VariableChangeEvent
from src.core.variables.variable import Variable
from src.core.variables.variable_factory import VariableFactory
# We only need the Variable class and VariableFactory for creating variables
...
class VariableStorageV2:
    ...
    def clear_scope(self, scope: VariableScope) -> None:
        ...
        for name, variable in variables:
            self._notify_variable_change(VariableChangeEvent(name, variable.get_value(), None, scope))
    ...
    def _notify_variable_change(self, event: VariableChangeEvent) -> None:
        ...
        Raises:
            Exception: If a listener raises an exception
        """
        errors = []
        ...
        # If there were errors, raise a combined exception
        if errors:
            if len(errors) == 1:
                raise errors[0]
            else:
                raise Exception(f"Multiple errors in variable change listeners: {[str(e) for e in errors]}")
    ...
    def _on_variable_change(self, event: VariableChangeEvent) -> None:
        ...
        Args:
            event: Variable change event
        """
        # Notify listeners directly with the event
        self._notify_variable_change(event)
